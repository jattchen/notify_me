...
> 仅顶层、直接面向用户的主 Agent 持续判断是否命中已启用的 Notify Me 内置条件：任务阻塞或需要立即介入的严重风险。只有判断命中时才按需读取并调用 Notify Me；普通问答、进度、完成、已获授权的常规敏感操作及任何子 Agent、委派 Agent、Ticket Worker 均不得调用。有效用户订阅由 Notify Me 的恢复 Hook 另行提供最小检查上下文。
...
4. 未绑定时，先询问用户是否已安装并打开 Bark；若没有，指导其完成安装并返回 Bark 首页。
5. 指导用户在 Bark 首页找到并复制完整推送 URL；自建服务用户使用自己的完整 URL。再次强调不要把地址发进聊天。
6. 由 Agent 打开可见终端运行隐藏输入；用户在不回显提示中输入或粘贴地址。只有无法安全交接时才提供一条不含地址、带真实安装路径的交互式 setup 命令。
7. 保存地址后发送一条默认 P1 测试。
8. 只有状态为 `delivered` 才询问用户手机是否实际收到；`queued/failed/expired` 继续诊断。
9. 用户确认手机收到后，展示两个内置条件的启用状态、订阅功能开关、P0–P3 优先级、P0–P2 默认效果和空置 P3。
10. 用户选择“使用默认配置”或“自定义”。自定义时先启停内置条件与订阅功能，再设置优先级；只有需要覆盖时才展开条件效果和完整铃声列表。
11. 展示最终配置摘要；可选发送每个实际使用效果的一条预览通知。
12. 解析并展示实际生效的全局 AGENTS 文件、精确托管块和原文件影响，解释“不写入则内置条件无法可靠触发”，请求明确授权。
13. 获得产品层和必要的平台授权后，仅执行一次 `agents-rule commit` 原子写入；随后只做逐字节 verify，不重复写入。
14. 展示两个订阅恢复 Hook 的精确定义和行为；引导用户在 `/hooks` 中审查并信任，或明确选择“不要 Hook，订阅仅当前上下文 best-effort”。
15. 运行 `doctor`；若当前任务在写入前已启动，状态进入 `restart-required`，明确说明当前任务不会热加载新 AGENTS 指令。
16. 用户启动一个新的顶层任务并显式调用一次 activation verify；验证当前生效指令来源、托管块版本和 Hook/降级模式后进入 `active`。
17. 用一句话确认绑定完成，并明确当前是“双 Hook 持久订阅恢复”还是“无 Hook 降级”模式。
...
5. **固定图标依赖公网托管**：现有 URL 只适合开发验证；正式发布必须使用不可变版本 URL、内容 SHA 检查和明确失效策略。
6. **全局规则可能与 Bark Push 冲突**：并存时可能双发，需要 Onboarding 检测和用户选择。
7. **优先级不等于实际打扰效果**：条件效果覆盖可以让 P1 比某个 P0 更响；所有界面必须同时展示优先级与解析后的 level/sound/volume。
8. **自然语言条件没有后台执行器**：用户订阅只在主 Agent 观察到条件满足时触发，不是独立监控服务。
9. **宿主 scope 合同尚需真机验证**：真实 Hook 输入和 CLI 环境能否稳定解析为同一顶层任务身份，是后续 Subscriptions & Recovery 阶段的硬门槛；缺失或冲突必须 fail closed。
10. **双 Hook 可能在相邻时点重复运行**：共享 `context_revision`、幂等查询和单项原子 claim 必须保证两种事件不会重复注入或重复补发。
11. **全局规则不是即时生效配置**：`AGENTS.override.md` 可能遮蔽普通文件，且修改通常要到新顶层任务才读取；Onboarding 必须展示实际生效文件并完成重启验证。

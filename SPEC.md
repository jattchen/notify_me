...
Notify Me 是一个以通知投递为核心的 Codex 插件。它保留 Bark Push 中经过验证的 Bark 配置、发送、隐私、任务隔离、事件去重、事项升级、网络队列与并发恢复能力，删除标题状态持久化、置顶、生命周期、任务控制 Hook 和兼容性恢复等耦合；全局托管规则可以协调主 Agent 使用宿主能力改准确当前任务名称。
...
1. 插件运行时不持久化或修改 Codex 任务标题、置顶和归档状态；任务改名只由顶层主 Agent 通过宿主能力执行，运行时仅只读标题或接收同轮改名的同名快照用于通知展示。
...
> 仅顶层、直接面向用户的主 Agent 持续判断 Notify Me 内置条件，并在任务目标明确或方向明显变化时使用宿主能力改准确当前顶层任务名称。同一轮既改名又通知时，先发起改名，再把同名快照传给通知入口，避免异步标题索引产生竞态；改名失败不阻断通知。有效用户订阅由 Notify Me 的恢复 Hook 另行提供最小检查上下文。
...
- 正常模式默认由运行时从 Codex 任务索引读取宿主当前任务的真实可见标题；只有主 Agent 在同一轮刚发起任务改名时，才把完全相同的新名称作为 `--task-title` 快照传给 `send` 或 `subscription trigger`，避免异步索引仍返回旧名称。改名失败或不可用时不延迟通知，并如实说明任务界面改名未完成。宿主无法可靠提供其他标题时只显示通知类型。
...
| 标题/body 最小化 | 保留并增加同轮标题快照 | 默认只读任务索引；刚发起改名时复用同名快照 |
...
| 标题状态持久化/置顶状态 | 删除 | 插件运行时范围外；任务改名由主 Agent 调用宿主能力 |
...
### 19.4 任务名称协调开工基线（2026-08-10）

1. 任务目标已经明确或方向发生明显变化时，由顶层主 Agent 立即使用宿主能力把当前任务名称改准确；普通进度和实现细节变化不触发改名，也不在最终答复前追加名称检查。
2. Hook 与 Notify Me 运行时不理解任务语义、不调用宿主改名能力，也不持久化任务标题状态。
3. 同一轮既改名又发送内置或订阅通知时，主 Agent 先发起改名，再把完全相同的新名称作为 `--task-title` 快照传给通知入口；其他通知继续只读宿主任务索引。
4. 宿主改名失败或能力不可用时不延迟通知，仍使用已经确定的标题快照，并向用户如实说明任务界面改名未完成。
5. `private` 模式继续忽略任务标题；订阅补发和显式 retry 继续复用首次触发时保存的不可变 payload。

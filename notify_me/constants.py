...
MANAGED_BLOCK = "\n".join(
    (
        ...
        "仅顶层、直接面向用户的主 Agent 持续判断是否命中 Notify Me 的 Blocking 条件：原定目标因等待用户授权、信息、选择或操作而无法继续。只有判断命中时才按需读取并调用 Notify Me；普通问答、进度、完成、已获授权的常规敏感操作及任何子 Agent、委派 Agent、Ticket Worker 均不得调用。",
...
SCHEMA_SQL = "\n".join(
    (
        ...
        "CREATE TABLE notifications (notification_id TEXT PRIMARY KEY, scope_key TEXT NOT NULL, condition_key TEXT NOT NULL CHECK (condition_key IN ('blocking')), event_key TEXT NOT NULL, event_state_key TEXT NOT NULL, effect_fingerprint TEXT NOT NULL, status TEXT NOT NULL CHECK (status IN ('sending', 'delivered', 'failed', 'deduplicated')), created_at REAL NOT NULL, updated_at REAL NOT NULL, attempts INTEGER NOT NULL DEFAULT 0, http_status INTEGER, last_error TEXT)",
        "CREATE UNIQUE INDEX notifications_event_identity ON notifications (scope_key, condition_key, event_key, event_state_key, effect_fingerprint)",
    )
)

...
4. `python3 ~/.grok/plugins/notify-me/scripts/notify_me.py test --dry-run`，再跑不带 `--dry-run` 的 `test`。完成标准：后者 `status=accepted`，且用户确认手机出现「Grok Notify Me」。
5. `python3 ~/.grok/plugins/notify-me/scripts/notify_me.py agents-rule plan`。把返回的 `block` 原文给用户看，问是否写入 `target`。用户同意前不得 commit。
6. 用户同意后：`python3 ~/.grok/plugins/notify-me/scripts/notify_me.py agents-rule commit --authorize`。完成标准：`ok=true`。告诉用户当前会话不会热加载，新开一局后规则才生效。
